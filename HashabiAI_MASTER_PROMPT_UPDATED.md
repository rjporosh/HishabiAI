...
Build a production-quality personal finance application named **HashabiAI** with the subtitle **Smart Personal Expense Manager**.
...
Official product spelling: **HashabiAI**.
...
window.HashabiAI.receiveNativeSpeech(text)
...
If upgrading from an older HashabiAI IndexedDB schema, migrate old stores safely.
...
A. `HashabiAI.html` — complete single-file offline-first application.
