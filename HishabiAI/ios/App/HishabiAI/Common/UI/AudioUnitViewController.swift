...
private let log = Logger(subsystem: "com.hishabiai.porosh.app.HishabiAI", category: "AudioUnitViewController")

@MainActor
public class AudioUnitViewController: AUViewController, AUAudioUnitFactory {
    var audioUnit: AUAudioUnit?
    
    var hostingController: HostingController<HishabiAIMainView>?
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try HishabiAIAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? HishabiAIAudioUnit else {
				log.error("Unable to create HishabiAIAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(HishabiAIParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = HishabiAIMainView(parameterTree: observableParameterTree)
